...
function firstUpper(cadena) {
    ...
    return cadena.replace(/^[a-z]{1}/g, function ($1) {
    ...
    niveles.forEach(nivel => {
        ...
        grados.forEach(grado => {
            ...
            actividades.forEach(actividad => {
                ...
                links.forEach(l => { // console.log(`${nivel}-${grado}-${actividad}---${l.link}`);
...
function generaBotonesOpciones(opciones) {
    ...
    if (_op.length > 2) {
        ...
        _op.forEach(l => { /* si el elemento ya existe, solamente le quita la clase de hidden 
      ...
      */
        ...
        for (let n of navbarPills.children) {
...
function cleanSlate(c) { // clean contenido
